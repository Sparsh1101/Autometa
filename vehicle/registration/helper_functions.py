...
def check_religion(name):
    religion = ["Hinduism", "Islam", "Christianity", "Sikhism", "Jainism", "Buddhism"]
    if name in religion:
        return True
    return False


def check_type_of_fam(name):
    type_of_fam = ["Nuclear", "Joint", "Extended"]
    if name in type_of_fam:
        return True
    return False


def check_education(name):
    education = [
        "Professional degree (Post graduate)",
        "Graduate (Bachelors)",
        "Higher Secondary certificate (12th Std)",
        "Secondary school certificate (SSC - 10th Std)",
        "Middle school (5th to 10th std)",
